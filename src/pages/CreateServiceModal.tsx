...
export function CreateServiceModal({ isOpen, onClose, onSuccess, editingService }: CreateServiceModalProps) {
  ...
  useEffect(() => {
    if (editingService) {
      ...
      setUploadedImageUrl(editingService.image || null);
    } else {
      resetForm();
    }
  ...
  // 🔥 NEW: Upload image to DigitalOcean first
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 🔥 CHANGED: Send image URL as string, not file
      const payload = {
...
const url = editingService
  ? `${import.meta.env.VITE_API_BASE_URL}/serviceRoutes/updateService?id=${editingService._id}`
  : `${import.meta.env.VITE_API_BASE_URL}/serviceRoutes/createService`;
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className="inline-flex items-center space-x-2 text-xs uppercase tracking-wider text-gray-400 mb-1">
                ...
                <span>{editingService ? 'Edit' : 'Create'} Service</span>
              </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500 font-light">Enter 0 for complimentary services</p>
            </div>
          </div>
          ...
          <div>
            ...
            {uploadingImage && (
              <p className="mt-2 text-xs text-blue-600 font-light">⏳ Uploading to DigitalOcean Spaces...</p>
            )}
          </div>
          ...
          <div className="flex items-center space-x-3 pt-4 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={loading || uploadingImage}
              className="flex-1 px-4 py-2.5 bg-gray-900 text-white rounded-lg hover:bg-gray-800 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center space-x-2 font-light transition-colors"
            >
              {loading ? (
                <>
